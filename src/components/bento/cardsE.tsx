...
/* ================================================================== */
/* CARD — Product Lens (patch v6, adapted from ProductLensPanel)       */
/* Exoskeleton sensor capability: REAL camera capture via getUserMedia.*/
/* SKU/product identification and catalog lookup are stubbed pending   */
/* a catalog-match endpoint — the captured frame is ready to POST.     */
/* ================================================================== */
...
export function ProductLensDemo() {
  ...
  /* stop all tracks when the card unmounts */
  useEffect(() => {
    return () => {
        ...
        >
          {capture ? (
            ...
            <span className="p-2 text-center text-[10px] leading-snug" style={{ color: 'var(--bk-text-3)' }}>
              {capture ? '' : 'Capture a product to identify it.'}
            </span>
          )}
        </div>
